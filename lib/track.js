...
Track.prototype.getStreamURL = function (qcastResponse) {
    ...
    spotify.trackUri(track, function (err, trackResponse) {
        if (err) qcastResponse.status(404).send('Bad Request');
        if (!trackResponse.uri) qcastResponse.status(404).send('response contained no "uri"');
        console.log('GET %s', trackResponse.uri);
        track._playSession = trackResponse;
        qcastResponse.jsonp({
            status: "ok",
            mp3url: trackResponse.uri
        }).send();
    });
};
