...
Playlist.prototype._performDiff = function(newRevision, oldRevision) {
  ...
  this.diff(oldRevision, function(err, diff) {
    // TODO(adammw): fire relevant events
...
Playlist.prototype.diff = function(revision, fn) {
  ...
  this._request('DIFF', { revision: revision.toString() }, function(err, res) {
    ...
    if (Array.isArray(diff.ops)) diff.ops.forEach(function(op) {
      if ('ADD' == op.kind || 'REM' == op.kind) {
        op.items = op.items.map(function(item) {
          return this.PlaylistItem(item.uri, item.attributes);
        }, this);
      }
    }, this);
    fn(null, diff);
