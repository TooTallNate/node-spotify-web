...
Playlist.create = function(spotify, attributes, fn) {
  ...
  request.send(requestArgs, function (err, res) {
    if (err) fn(err);
    var uri = res.result.uri.toString();
    debug('playlist created - uri = %s', uri);

    spotify.user.rootlist().add(uri, function(err, resp) {
      if (err) return debug('playlist failed to be added to rootlist', err);
      debug('playlist added to rootlist');
    });

    var playlist = new Playlist(spotify, uri);
...
function Playlist (spotify, uri) {
  ...
  if (!(uri instanceof SpotifyUri) || SpotifyUri.PLAYLIST_TYPES.indexOf(uri.type) === -1) {
    throw new Error('Invalid URI type: ' + uri.type);
  }

  this._hm_uri = 'hm://playlist/user/' + uri.user + '/' + uri.type;
  if (uri.sid != uri.type) {
    this._hm_uri += '/' + uri.sid;
  }
  this.type = uri.type; // e.g. playlist, starred, rootlist
...
Playlist.prototype._performDiff = function(newRevision, oldRevision) {
  ...
  this.diff(oldRevision, (function(err, diff) {
    if (err) return this.contents.emit('error', err);
 ...
 *
 * @param {Object|PlaylistAttributes} attributes
 * @param {Function} fn callback function
 */ 
Playlist.prototype.updateAttributes = function(attributes, fn) {
    ...
    if (Array.isArray(diff.ops)) {
      diff.ops.forEach(function(op) {
        ['add', 'rem'].forEach(function(kind) {
          ...
          op[kind].items = op[kind].items.map(function(item, i) {
            var playlistItem = new this.PlaylistItem(item.uri, item.attributes);
            playlistItem.index = op[kind].fromIndex + i;
            playlistItem.revision = (kind == 'add') ? diff.toRevision : diff.fromRevision;
            return playlistItem;
          }, this);
        }, this);
      }, this);
...
Playlist.prototype.contents = function(offset, length, fn) {
  ...
  this._request(function(err, res) {
    if (err) return fn(err);
    var contents = new this.PlaylistContents();
    try {
    ...
    try {
      var attributes = new this.PlaylistAttributes();
      attributes.parse(res.result);
      this._attributesCache = attributes;
    } catch(e) {
      debug("failed to parse attributes", e);
    }
 ...
 * @param {Number} prepend (optional) set to true to add the item to the start of the playlist rather than the end
 * @param {Object} attributes (optional)
 * @param {Function} fn callback function
 * @api public
 */
Playlist.prototype.add = function(items, prepend, attributes, fn) {
  // argument surgery
  if (!Array.isArray(items)) {
    items = [ items ];
  }
  if ('function' === typeof prepend) {
    fn = prepend;
    prepend = attributes = null;
  }
  if ('function' === typeof attributes) {
    fn = attributes;
    attributes = null;
  }
  if (null === prepend || 'undefined' == typeof prepend) { prepend = false; }
  fn = util.wrapCallback(fn, this);
  ...
  items = items.map(function(item) {
    return {
      ...
      attributes: (item.attributes) ? item.attributes : (attributes || {
        added_by: spotify.username
      })
    };
  });
 ...
 * @api public
 */
Playlist.prototype.remove = function(items, fn) {
  // argument surgery
  if (!Array.isArray(items)) {
    items = [ items ];
  }
  ...
  // perform request
  var HermesRequest = this._spotify.HermesRequest;
  var request = new HermesRequest("REMOVE", this._hm_uri);
  var payload = items.map(function(item) {
    return (item.uri) ? item.uri.toString() : item;
  }).join(',');
  request.send(new Buffer(payload).toString('base64'), fn);
};
