...
PlaylistContents.prototype.parse = function(data) {
  ...
  if (data.contents.items && data.contents.items.length) {
    data.contents.items.forEach(function(item, i) {
      var playlistItem = new PlaylistItem(item.uri, item.attributes);
      playlistItem.index = this.offset + i;
      playlistItem.revision = this.revision;
      this.push(playlistItem);
    }, this);
  }
};
