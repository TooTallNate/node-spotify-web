 ...
 * @param {String} revision (optional) playlist revision
 * @param {Function} fn (optional) callback function for metadata request
 */

var Playlist = exports = module.exports = function(spotify, uri, revision, fn){
  // argument surgery
...
Playlist.create = function(spotify, attrs, fn) {
  ...
  spotify.sendProtobufRequest(args, function(err, res) {
    ...
    var playlist = new Playlist(spotify, playlistUri, revision);
...
Playlist.prototype.addItems = function(itemUri, addFirst, fn) {
  ...
  debug('Playlist[uri=%s]#addItem(%j, %j)', this._uri, itemUri, addFirst);
...
Playlist.prototype.removeItem = function(removeItem, fetchData, fn) {
  ...
  if (fetchData) {
    ...
    this.get(function(err, obj) {
      self.removeItem(removeItem, false, fn);
    });
    return;
  }
