 ...
 *
 * By default, the privacy of the Playlist will be secret, 
 * To make it public, call `playlist.publish()`.
 *
 * @param {Object} spotify Spotify instance
...
Playlist.create = function(spotify, attrs, fn) {
  //TODO: add argument to make it public (syncpublished parameter)
  if (!(spotify instanceof Spotify)) throw new Error('Spotify instance required');
...
/**
 * Publish a playlist to the user's publishedrootlist
 *
 * If a playlist is published to the user's publishedrootlist it has the effect of making the playlist public
 *
 * @param {Boolean} secret (optional) set true to unpublish (make secret)
 * @param {Function} fn callback function
 * @api public
 */
Playlist.prototype.publish = function(secret, fn) {
  // argument surgery
  if ('function' == typeof secret) {
    fn = secret;
    secret = false;
  }

  debug('Playlist[uri=%s]#publish(%j)', this._uri, secret);

  // Add or remove the playlist to the publishedrootlist
  var spotify = this._spotify;
  var pubrootlist = new Playlist(spotify, 'spotify:user:' + spotify.username + ':publishedrootlist');
  //TODO: check first if it is already in the publishedrootlist so we don't get duplicates
  if (secret) {
    pubrootlist.remove(this._uri, fn);
  } else {
    pubrootlist.add(this._uri, true, fn);
  }
};

/**
 * Unpublish (make secret) the playlist
 * Helper function for Playlist#publish(true)
 *
 * @param {Function} fn callback function
 * @api public
 */
Playlist.prototype.unpublish = function(fn) {
  this.publish(true, fn);
};
