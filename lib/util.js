...
/**
 * Converts Revision Buffer to String
 */

exports.revision2string = function(revision) {
  var revNum = revision.readUInt32BE(0);
  var hash = exports.gid2id(revision.slice(4));
  return revNum+','+hash;
}
...
exports.uriType = function (uri) {
  ...
  if (len >= 3 && 'local' == parts[1]) {
    ...
    return parts[3];
  } else if (len >= 4 && ('starred' == parts[3] || 'rootlist' == parts[3] || 'publishedrootlist' == parts[3])) {
    return 'playlist'; 
  } else if (len >= 3) {
    return parts[1];
  } else {
