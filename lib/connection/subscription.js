...
function Subscription(connection, uri) {
  debug('Subscription(%j, %j)', name, args);
 ...
 * @api private
 */
Subscription.prototype._onsubscribed = function(err, subscription) {
...
Subscription.prototype.subscribe = function() {
  ...
  if (-1 === subscriptions.indexOf(this)) subscriptions.push(this);
  for (var i = 0, l = subscriptions.length; i < l; i++) {
    var subscription = subscriptions[i];
    if (subscription != this && subscriptions[i].uri == this.uri) return;
  }
 ...
 * and call the unsubscribe handler if it's the first active subscription for this uri
 */
