  ...
  // general case
    ...
    if (header.userFields) {
      header.userFields.forEach(function(field) {
        this.userFields[field.name] = field.value;
      }, this);

      if ('MC-Cache-Policy' in this.userFields)
        this.cachePolicy = this.userFields['MC-Cache-Policy'].toString();
      if ('MC-ETag' in this.userFields)
        this.etag = this.userFields['MC-ETag'];
      if ('MC-TTL' in this.userFields)
        this.ttl = Number(this.userFields['MC-TTL'].toString());
    }
