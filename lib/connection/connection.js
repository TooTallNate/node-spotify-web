...
var EXPORT_NAMESPACES = [Request, Response, HermesRequest, HermesResponse];

EXPORT_NAMESPACES.forEach(function (namespace) {
  SpotifyConnection[namespace.name] = namespace;
});
...
function SpotifyConnection(spotify) {
  ...
  // handle events
  this.on('heartbeat', this._onheartbeat.bind(this));
  this.on('flush', this._onflush.bind(this));

  // bind exported namespaces into object
  EXPORT_NAMESPACES.forEach(util.bindNamespace.bind(null, this));
}
...
SpotifyConnection.prototype._startHeartbeat = function () {
  ...
  this._heartbeatId = setInterval(fn, this.emit.bind(this, 'heartbeat'));
};
...
SpotifyConnection.prototype.connect = function(url, fn) {
  ...
  ['open', 'close', 'message'].forEach(function(event) {
    this.ws.on.bind(event, this.emit.bind(this, event));
  }, this);
...
SpotifyConnection.prototype.sendConnect = function (fn) {
  ...
  this.Request('connect').send(args, this._onconnect.bind(this));
};
