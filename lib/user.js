...
User.prototype.rootlist = function(type) {
  if (type !== 'rootlist' && type !== 'publishedrootlist') {
    type = (this.isCurrentUser) ? 'rootlist' : 'publishedrootlist';
  }

  return this._spotify.get(['spotify', 'user', this.username, type].join(':'));
};
...
User.prototype.starred = function() {
  return this._spotify.get(['spotify', 'user', this.username, 'starred'].join(':'));
};
