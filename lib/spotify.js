...
Spotify.prototype.sendPong = function(ping) {
  this.agent
  .get('http://spflash.herokuapp.com/generic/get')
  .query({ ping: ping })
  .set({ 'User-Agent': this.userAgent })
  .end(function (err, res) {
    ...
    if (res.status == 200) {
      this.sendCommand('sp/pong_flash2', [ res.text ]);
    } else {
      return this.emit('error', res.body.status);
    }
  }.bind(this));
};
...
Spotify.prototype.library = function (user, from, length, fn) {
  ...
  var self = this;
  var hm = 'hm://collection-web/v1/' + user + '/songslist?start=' + from + '&length=' + length;
  var header = {
      method: "GET",
      uri: hm
  };
