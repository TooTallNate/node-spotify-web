...
[SpotifyConnection, SpotifyUri, Metadata, User].forEach(util.recursiveExport, Spotify);
...
Spotify.prototype._objectify = function(obj, parent) {
  ...
  for (var i = 0, il = exports.length; i < il; i++) {
    ...
    if (type.acceptedSchemas && Array.isArray(type.acceptedSchemas)) {
      console.log(type.name, 'accepts:', type.acceptedSchemas);
      for (var j = 0, jl = type.acceptedSchemas.length; j < jl; j++) {
        var schema = type.acceptedSchemas[j];
        if (obj instanceof schema) {
          // TODO(adammw): make sure passing in a parent like this makes sense,
          // perhaps we could better do it by looking at the $inject again
