...
{
  var spot = this;
  request('http://ping-pong.spotify.nodestuff.net/' + ping.split(" ").join("-"), function(error, response, body)
  {
    var json = JSON.parse(body);
    if (json.status == 100)
    {
      pong = json.pong.split("-").join(" ");
      spot.sendCommand('sp/pong_flash2', [pong]);
    }
  });
