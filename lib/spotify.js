...
var mercury = schemas.mercury.build('spotify.mercury.proto');
var MercuryMultiGetRequest = mercury.MercuryMultiGetRequest;
var MercuryMultiGetReply = mercury.MercuryMultiGetReply;
var MercuryRequest = mercury.MercuryRequest;
var MercuryReply = mercury.MercuryReply;
...
var playlist4changes = schemas.playlist4changes.build('spotify.playlist4.proto');
var ListDump = playlist4changes.ListDump;

var toplist = schemas.toplist.build();
...
Spotify.prototype.metadata = function (uris, fn) {
  ...
  if (requests.length == 1) {
    data = new MercuryRequest(requests[0]).toArrayBuffer().toString('base64');
    args.push(data);
  } else {
    data = new MercuryMultiGetRequest({ request: requests }).toArrayBuffer().toString('base64');
    var header = new MercuryRequest({
      body: "GET",
	    contentType: "vnd.spotify/mercury-mget-request",
	    uri: "hm://metadata/"+mtype+"s"
    }).toArrayBuffer().toString('base64');
...
Spotify.prototype.playlist = function (uri, from, length, fn) {
  ...
  if(uri.indexOf('"hm://') == -1) {
    ...
    var user = parts[2];
    var id = parts[4];
    hm = 'hm://playlist/user/' + user + '/playlist/' + id +
      '?from=' + from + '&length=' + length;
  } else {
    hm = uri + '?from=' + from + '&length=' + length;
  }

  var req = { body: 'GET', uri: hm };
  var data = new MercuryRequest(req).toArrayBuffer().toString('base64');
  var args = [ 0, data ];
