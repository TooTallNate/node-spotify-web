 ...
 * @param {Function} listener callback function called when changes occur
 * @param {Function} fn callback function called when subscription created
 * @api public
 */

Spotify.prototype.subscribe = function (uris, listener, fn) {
  // argument surgery
  if ('function' == typeof uris) {
    fn = listener;
    listener = uris;
    uri = 'spotify:user:' + this.username + ':rootlist';
  }
  if (!Array.isArray(uris)) {
    uris = [uri];
  }
