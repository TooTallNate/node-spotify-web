...
var SelectedListContent = playlist4changes['spotify.playlist4.proto.SelectedListContent'];
var playlist4service = schemas.playlist4service;
var SubscribeRequest = playlist4service['spotify.playlist4.proto.SubscribeRequest'];
var PlaylistModificationInfo = playlist4service['spotify.playlist4.proto.PlaylistModificationInfo'];
var pubsub = schemas.pubsub;
var Subscription = pubsub['spotify.hermes.pubsub.proto.Subscription'];

var playlist4ops = schemas.playlist4ops;
var OpList = playlist4ops['spotify.playlist4.proto.OpList'];

var playlist4service = schemas.playlist4service;
var CreateListReply = playlist4service['spotify.playlist4.proto.CreateListReply'];
var ModifyReply = playlist4service['spotify.playlist4.proto.ModifyReply'];
...
Spotify.prototype._onmessagecommand = function (command, args) {
  if ('do_work' == command) {
    ...
    // ignore...
  } else if ('hm_b64' == command) {
    var header = this._parse(MercuryRequest, new Buffer(args[1], 'base64'));
    debug('got "hm_b64" header: %j', header);
    if (Array.isArray(this._callbacks[header.uri])) {
      if (header.statusCode == 200) {
        this._callbacks[header.uri].forEach(function(fn){
          fn.call(this, null, args);
        });
      } else {
        this._callbacks[header.uri].forEach(function(fn){
          fn.call(this, args);
      ...
      debug('unhandled "hm_b64" with args: %j', args);
    }
  } else {
    // unhandled message
...
Spotify.prototype.sendProtobufRequest = function(req, fn) {
  ...
  var isMultiResponseSchema = (responseSchema && !responseSchema.hasOwnProperty('parse'));
  ...
  debug('payloadSchema: %j', payloadSchema);
...
/**
 * Subscribe to changes to a playlist or rootlist.
 *
 * @param {Array|String} uris (optional) A single URI, or an Array of URIs to subscribe to. 
 *                                      If not specified, will default to the logged-in user's rootlist
...
Spotify.prototype.subscribe = function (uris, listener, fn) {
  // argument surgery
  if ('function' == typeof uris || 'undefined' == typeof uris) {
    fn = listener;
    listener = uris;
    uris = 'spotify:user:' + this.username + ':rootlist';
  }
  if (!Array.isArray(uris)) {
    uris = [uris];
  }
  
  debug('subscribe(%j)', uris);
  ...
  var cb = function(err, args) {
    ...
    var data = self._parse(PlaylistModificationInfo, new Buffer(args[2], 'base64'));
    listener(null, data);
  };

  uris.forEach(function(uri) {
    var parts = uri.split(':');
    if (parts[1] != 'user' || (parts[3] != 'playlist' && parts[3] != 'rootlist')) {
      throw new Error('URI must be a playlist or rootlist URI')
    }
    var user = parts[2];
    var id = parts[4];
    ...
    if ('function' == typeof listener) {
      // make sure uris end with a slash - needed to ensure callbacks match correctly
      if (hm.substr(-1) != '/') {
        hm += '/';
      }

      // add callback into map with url as the key
      if (!self._callbacks[hm]) {
        self._callbacks[hm] = [];
      }
      debug('adding callback listener for url: %s', hm);
      self._callbacks[hm].push(cb);
    }
  });
...
/*
 * Add an item to the rootlist
 *
 * @param {String} itemUri the uri of the item to add
 * @param {Boolean} addFirst add the item to the start instead of the end
 * @param {Function} fn callback function
 * @api public
 */

Spotify.prototype.addToRootlist = function(uri, addFirst, fn) {
  // argument surgery
  ...
  debug('addToRootlist(%j, %j)', uri, addFirst);

  // TODO: validate uri type

  var hm = 'hm://playlist/user/' + this.username + '/rootlist';
  ...
  this.sendProtobufRequest({
    ...
    // TODO: return rootlist object

    fn(null, {revision: revision});
  });
};
...
Spotify.prototype.addToPlaylist = function(itemUri, playlistUri, addFirst, fn) {
  // argument surgery
  if ('function' == typeof addFirst) {
    fn = addFirst;
    addFirst = false;
  }
  debug('addToPlaylist(%j, %j, %j)', itemUri, playlistUri, addFirst);

  var playlistType = Spotify.uriType(playlistUri);
  if ('playlist' != playlistType) {
    throw new Error('Must pass a "playlist" URI, got ' + JSON.stringify(playlistType));
  }
  ...
  if (Array.isArray(itemUri)) {
    ...
    this.sendProtobufRequest({
      ...
      debug('playlist revision = %j', revision);

      var playlist = new Playlist(playlistUri, revision);
      tag(this, playlist);
      fn(null, playlist);
    });
  } else {
    // add a single item
    debug('adding single item to playlist');
    if (addFirst) {
      hm += '?add_first=true';
    }
...
/**
 * Recurse through a playlist to find the position of the item in the playlist
 * 
 * @param {String|Null} playlistUri the uri of the playlist to scan or null for the rootlist
 * @param {String} searchUri the uri of the item to search for
 * @param {Function} fn callback function
 * @api private
 */

Spotify.prototype._recursePlaylist = function(playlistUri, searchUri, fn) {
  var self = this;
  var from = 0;
  var length = 100;
  var loadFunction = this.playlist;

  // recurse the rootlist if no playlist specified
  if (!playlistUri) {
    loadFunction = this.rootlist;
    playlistUri = null;
  }

  (function recursePlaylist(){
    debug('recursing through playlist (%j, %j)', from, length);

    loadFunction.call(self, playlistUri, from, length, function(err, playlist) {
      if (err) return fn(err);

      // search
      for (var i = 0, l = playlist.contents.items.length; i < l; i++) {
        var item = playlist.contents.items[i];
        if (item.uri == searchUri) {
          playlistIdx = playlist.contents.pos + i;
          debug('found item in playlist at position %d', playlistIdx);
          return fn(null, {
            fromIndex: playlistIdx,
            length: 1,
            revision: util.revision2string(playlist.revision)
          });
        }
      }
      
      // load next page or error out
      if (playlistIdx == -1) {
        if (rootlist.contents.truncated) {
          from += length;
          return recursePlaylist();
        } else {
          return fn(new Error('Could not find item in playlist'));
        }
      }
    });
  })();
...
/**
 * Remove an item from the rootlist
 *
 * @param {String|Object} removeItem the uri of the item to remove or an object specifying the position and length
 * @param {String} revision the revision of the rootlist to work off
 * @param {Function} fn callback function
 * @api public
 */

Spotify.prototype.removeFromRootlist = function(removeItem, revision, fn) {
  // argument surgery and validation
  var removeArgs= {};
  if('string' == typeof removeItem) {
    if ('function' == typeof revision) {
      fn = revision;
      revision = null;
    }

    // come back after the position in the rootlist has been found
    var self = this;
    return this._recursePlaylist(null, removeItem, function(err, res){
      self.removeFromRootlist(res, res.revision, fn);
    });
  } else if ('function' == typeof revision) {
    throw new Error('Revision must be specified when removeItem is not a uri');
  } else if (!removeItem.hasOwnProperty('fromIndex') || !removeItem.hasOwnProperty('length')) {
    throw new Error('FromIndex and Length must be specified on removeItem');
  } else {
    removeArgs.fromIndex = removeItem.fromIndex;
    removeArgs.length = removeItem.length;
  }

  debug('removeFromRootlist(%j, %j, %j)', removeItem, removeArgs, revision);

  var hm = 'hm://playlist/user/' + this.username + '/rootlist?syncpublished=true&revision=' + revision;
 ...
 *
 * @param {String|Object} removeItem the uri of the item to remove or an object specifying the position and length
 * @param {String} playlistUri the uri of the playlist to remove from
 * @param {String} revision the revision of the playlist to work off
 * @param {Function} fn callback function
 * @api public
 */

Spotify.prototype.removeFromPlaylist = function(removeItem, playlistUri, revision, fn) {
  // argument surgery and validation
  var removeArgs= {};
  if('string' == typeof removeItem) {
    if ('function' == typeof revision) {
    ...
    // come back after the position in the playlist has been found
    return this._recursePlaylist(playlistUri, removeItem, function(err, res){
      this.removeFromPlaylist(res, playlistUri, res.revision, fn);
    });
  } else if ('function' == typeof revision) {
    throw new Error('Revision must be specified when removeItem is not a uri');
  } else if (!removeItem.hasOwnProperty('fromIndex') || !removeItem.hasOwnProperty('length')) {
  ...
  debug('removeFromPlaylist(%j, %j, %j, %j)', removeItem, removeArgs, playlistUri, revision);
  ...
  var user = parts[2];
  var id = parts[4];
  var hm = 'hm://playlist/user/' + user + '/playlist/' + id + '?syncpublished=true&revision=' + revision;
...
Spotify.prototype.createPlaylist = function(attrs, fn) {
  ...
  var args = {
    header: {
      method: 'PUT',
      uri: 'hm://playlist/user/' + this.username
    },
    payload: {
      ops: [{
  ...
  var self = this;
  this.sendProtobufRequest(args, function(err, res) {
    if (err) return fn(err);
    ...
    self.addToRootlist(playlistUri, true, function(err, res){
      if (err) return fn(err);
...
Spotify.prototype.modifyPlaylist = function(uri, attrs, fn) {
  debug('modifyPlaylist(%j, %j)', uri, attrs);
  ...
  var user = parts[2];
  var id = parts[4];
  var hm = 'hm://playlist/user/' + user + '/playlist/' + id + '?syncpublished=true';

  var newAttributes = { values: {} };
  if (attrs.hasOwnProperty('name'))
    newAttributes.values.name = attrs.name;
  if (attrs.hasOwnProperty('deletedByOwner'))
    newAttributes.values.deletedByOwner = attrs.deletedByOwner;
  // TODO: Other attributes
  ...
  this.sendProtobufRequest(args, function(err, res) {
    ...
    var playlistUri = res.uri.toString();
    var revision = util.revision2string(res.revision);
    debug('playlist modified, revision = %j', revision);
 ...
 * Playlist is not actually deleted, just removed from the user's rootlist.
...
Spotify.prototype.deletePlaylist = function(uri, fn) {
  debug('deletePlaylist(%j, %j)', uri);
  ...
  var self = this;
  this.removeFromRootlist(uri, null, function(err, res) {
    ...
    if (user == self.username) {
      ...
      self.modifyPlaylist(uri, { deletedByOwner: true }, function(err, res) {
        if (err) return fn(err);
        fn(null);
      });
    } else {
