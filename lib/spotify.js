...
Spotify.prototype.metadata = function (uris, fn, loadMetadata) {
  ...
  var typeMapping = {
    ...
    'starred': this.Playlist,
    'rootlist': this.Playlist,
    'publishedrootlist': this.Playlist,
    'user': this.User
  };
  var objects = uris.map(function (uri) {
    uri = new SpotifyUri(uri);

    if (!(uri.type in typeMapping)) {
      debug('unhandled uri type: %s', uri.type);
      return {
        uri: uri,
        type: uri.type
      };
    ...
    var err = null;
    try {
      object = new typeMapping[uri.type](uri);
    } catch (e) {
      err = e;
    }
...
/**
 * Gets a user's stored playlists
 *
 * @param {String} user (optional) the username to get the rootlist for.
 * @api public
 */

Spotify.prototype.rootlist = function (user) {
  if (!user) user = this.username;
  debug('rootlist(%j)', user);

  var rootlistType = (user == this.username) ? 'rootlist' : 'publishedrootlist';

  return this.Playlist(['spotify', 'user', user, rootlistType].join(':'));
};
