...
Spotify.prototype.createPlaylist = function(attrs, fn) {
  ...
  this.sendProtobufRequest(args, function(err, res) {
    ...
    debug('playlist created - %j : %j', playlistUri, res);

    // Add the playlist to the rootlist
    self.sendProtobufRequest({
      header: {
        method: 'ADD',
        uri: 'hm://playlist/user/' + self.username + '/rootlist?add_first=true'
      },
      payload: playlistUri,
      payloadSchema: null,
      responseSchema: null
...
Spotify.prototype.modifyPlaylist = function(uri, attrs, fn) {
  ...
  this.sendProtobufRequest(args, function(err, res) {
    ...
    debug('playlist modified - %j : %j', playlistUri, res);
