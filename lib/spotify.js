...
Spotify.prototype.deletePlaylist = function(uri, fn) {
  ...
  var user = parts[2];
  var id = parts[4];
  var playlistHm = 'hm://playlist/user/' + user + '/playlist/' + id + '?syncpublished=true';
  var rootlistHm = 'hm://playlist/user/' + this.username + '/rootlist?syncpublished=true';

  var from = 0;
  var length = 100;
  var playlistIdx = -1;
  var revision;
  var loadRootlistRecursive = function(fn) {
    debug('recursing through rootlist (%j, %j)', from, length);
    self.rootlist(user, from, length, function(err, rootlist) {
      if (err) return fn(err);

      // update revision
      revision = util.revision2string(rootlist.revision);

      // search
      for (var i = 0, l = rootlist.contents.items.length; i < l; i++) {
        var item = rootlist.contents.items[i];
        if (item.uri == uri) {
          playlistIdx = rootlist.contents.pos + i;
          debug('found playlist in rootlist at position %d', playlistIdx);
          return fn(null, playlistIdx);
        }
      }
      ...
      if (playlistIdx == -1) {
        if (rootlist.contents.truncated) {
          from += length;
          return loadRootlistRecursive(fn);
        } else {
          return fn(new Error('Could not find playlist in rootlist'));
        }
      }
    });
  };

  loadRootlistRecursive(function(err, playlistIdx) {
    if (err) return fn(err);

    debug('modifying rootlist - removing from index %d - rootlist revision %j', playlistIdx, revision);

    self.sendProtobufRequest({
      header: {
        method: 'MODIFY',
        uri: rootlistHm + '&revision=' + revision
      },
      payload: {
        ops: [{
          kind: 'REM',
      ...
      debug('rootlist response: revision = %j', util.revision2string(res.revision));
