...
/**
 * Gets the metadata from a Spotify "starred" URI.
 *
 * @param {String} uri (optional) starred uri. defaults to current users starred
...
Spotify.prototype.starred = function (uri, from, length, fn) {
  // argument surgery
  if ('function' == typeof uri) {
    fn = uri;
    uri = 'spotify:user:' + this.settings.username + ':starred';
    from = length = null;
  ...
  debug('starred(%j, %j, %j)', uri, from, length);
  var self = this;
  var parts = uri.split(':');
  var user = parts[2];
  var hm = 'hm://playlist/user/' + user + '/starred' +
