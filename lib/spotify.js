...
Spotify.prototype.isTrackAvailable = function (track, country) {
    /*
   * Either we allow that the track has a restriction or if it does, we must follow that restriction.
   * return where this restriction has our membership and a restriction
   * we DONT want to return any results here otherwise it means it's forbidden!
  */
  var memberships = {premium: 3, unlimited: 1, free: 0};
  var membership = (this.accountType in memberships ? memberships[this.accountType] : 4);
  return !track.restriction || !track.restriction.find((x) => membership in x.catalogue && 
    !!~((x.countriesForbidden || '').match(/[A-Z]{2}/g) || []).indexOf(country)
  );
};
...
Spotify.prototype.recurseAlternatives = function (track, country, fn) {
  ...
  if (this.isTrackAvailable(track, country)) {
    ...
    for (var i = 0; i < tracks.length; i++) {
      debug('checking for an alternative track %j', track.uri);
      track = tracks[i];
      if (this.isTrackAvailable(track, country)) {
        return done();
      }
  ...
  // not playable
  process.nextTick(function () {
    //We know from here that we cannot get the track in this country, since the old error code isn't intended for the country, 
    //I've made my own 'SpotifyError' to show that the country cannot use the track (I have also appended these changes in error.js)
    fn(new SpotifyError([12, 2, 'Track is not playable in this country "' + country + '".', null]), track);
  });
};
...
function has (array, val) {
  return Array.isArray(array) && !!~array.indexOf(val);
}
