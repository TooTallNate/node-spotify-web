...
var MercuryMultiGetRequest = schemas.build('spotify.mercury.proto.MercuryMultiGetRequest');
var MercuryMultiGetReply = schemas.build('spotify.mercury.proto.MercuryMultiGetReply');
var MercuryRequest = schemas.build('spotify.mercury.proto.MercuryRequest');
var MercuryReply = schemas.build('spotify.mercury.proto.MercuryReply');
...
var ListDump = schemas.build('spotify.playlist4.proto.ListDump');

var Toplist = schemas.build('Toplist');
...
Spotify.prototype.metadata = function (uris, fn) {
  ...
  if (requests.length == 1) {
    data = new MercuryRequest(requests[0]).toBuffer().toString('base64');
    args.push(data);
  } else {
    data = new MercuryMultiGetRequest({
      request: requests
...
Spotify.prototype.playlist = function (uri, from, length, fn) {
  ...
  var data = new MercuryRequest(req).toBuffer().toString('base64');
  var args = [ 0, data ];
...
Spotify.prototype.rootlist = function (user, from, length, fn) {
  ...
  this.sendCommand('sp/hm_b64', args, function (err, res) {
    ...
    if (data.length >= 2) {
      ...
      var header = MercuryReply.decode(new Buffer(res.result[0], 'base64'));
    }
    fn(err, obj);
  });
};
...
Spotify.prototype.isTrackAvailable = function (track, country) {
  ...
  if (Array.isArray(track.restriction)) {
    for (var i = 0; i < track.restriction.length; i++) {
      ...
      var isAllowed = restriction == null || has(allowed, country);
