...
Spotify.prototype._onmessagecommand = function (command, args) {
  if ('do_work' == command) {
    ...
    var header = self._parse(MercuryRequest, new Buffer(args[1], 'base64'));
    debug('got "hm_b64" header: %j', header);
    if (this._callback[header.uri]) {
      if (header.statusCode == 200) {
        this._callback[header.uri].forEach(function(fn){
 ...
 *
 * @param {Array|String} uri (optional) A single URI, or an Array of URIs to subscribe to. 
...
Spotify.prototype.subscribe = function (uri, listener, fn) {
  // argument surgery
  ...
  uri.forEach(function() {
    ...
    var user = parts[2];
    var id = parts[4];
    var hm = 'hm://playlist/user/' + user + (id) ? '/playlist/' + id : '';
    hm_uris.push(hm);
