...
var SelectedListContent = schemas.build('spotify.playlist4.proto.SelectedListContent');
...
Spotify.prototype.sendProtobufRequest = function(req, fn) {
  ...
  var loadSchema = function(schema, dontRecurse) {
    if ('string' === typeof schema) {
      ...
      var schema = schemas.build('spotify.' + schemaName[0] + '.proto.' + schemaName[1]);
    } else if (schema && !dontRecurse) {
  ...
  var data = new MercuryRequest(header).toBuffer().toString('base64');
  args.push(data);
  ...
  if (payload && payloadSchema) {
    data = new payloadSchema(req.payload).toBuffer().toString('base64');
    args.push(data);
  }
