...
var bartender = schemas.bartender;
var StoryRequest = bartender['spotify.bartender.proto.StoryRequest'];
var StoryList = bartender['spotify.bartender.proto.StoryList'];
...
Spotify.prototype.createplaylist = function (user, name, fn) {
  ...
  var request = MercuryRequest.serialize({ body: 'PUT', uri: hm }).toString('base64');
  // kind values can be found in playlist4ops schema
  
  var data = playlist4ops['spotify.playlist4.proto.Op'].serialize({kind:6, updateListAttributes : {newAttributes : { values : { name : name}}}});
  ...
  this.sendCommand('sp/hm_b64', args, function (err, res) {
	...
	if (data.length >= 2) {
      // success!
      obj = self._parse(playlist4service['spotify.playlist4.proto.CreateListReply'], new Buffer(res.result[1], 'base64'));
