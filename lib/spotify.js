...
/**
 * Gets the user's starred tracks
 *
 * @param {String} user (optional) the user to find starred tracks for. defaults to the logged-in user.
 * @param {Number} from (optional) the start index. defaults to 0.
 * @param {Number} length (optional) number of tracks to get. defaults to 100.
 * @param {Function} fn callback function
 * @api public
...
Spotify.prototype.starred = function (user, from, length, fn) {
  ...
  var starred = new Playlist(this, 'spotify:user:' + user + ':starred');
  starred.get(from, length, fn);
};
...
Spotify.prototype.rootlist = function (user, from, length, fn) {
  ...
  var rootlist = new Playlist(this, 'spotify:user:' + user + ':rootlist');
  rootlist.get(from, length, fn);
};

/**
 * Add a callback to a Mercury subscription URI
 *
 * @param {Array|String} uris A single Mercury URI, or an Array of Mercury URIs to add callbacks for
 * @param {Function} listener (optional) callback function called when changes occur
 * @api public
 */

Spotify.prototype.addSubscriptionCallback = function (uris, listener) {
  debug('addSubscriptionCallback(%j)', uris);
  ...
  uris.forEach(function(hm){
    // ensure all uris end with a trailing slash
    if (hm.substr(-1) != '/') {
      hm += '/';
    }

    // add listener functions as callbacks
    if ('function' == typeof listener) {
      if (!self._callbacks[hm]) {
        debug('no callbacks defined for %s, creating new callback array', hm);
        self._callbacks[hm] = [];
      }
      self._callbacks[hm].push(listener);
    }
  });
};
...
Spotify.prototype.createPlaylist = function(attrs, fn) {
  ...
  this.sendProtobufRequest(args, function(err, res) {
    ...
    var rootlist = new Playlist(self, 'spotify:user:' + self.username + ':rootlist');
    rootlist.addItem(playlistUri, true, function(err, res) {
      if (err) return fn(err);
