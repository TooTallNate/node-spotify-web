...
function Spotify () {
  ...
  this.userAgent = 'Mozilla/5.0 (X11; Linux i686) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1650.39 Safari/537.36';
  ...
  this.clientVersion = 62300110; // Deployed at: 2013-12-20 11:22 UTC, Client version: 0.6.23.110, Hash: ba2a343
...
Spotify.prototype._onsecret = function (err, res) {
  ...
  console.log('login CSRF token %j', secret);
  console.log('trackingId %j', trackingId);

  var creds = this.creds;
  delete this.creds;
  creds.type = 'sp';
  creds.secret = secret;
  creds.trackingId = trackingId; // (In my case was "f055b1288b248507179d07337de7a678b07a1687")
  creds.cf = "";
  creds.f = "contextual";
  creds.s = "direct";
...
Spotify.prototype._openWebsocket = function (err, res) {
  ...
  console.log(url);
  //var url = 'wss://lon2-linkap-a3.ap.spotify.com/';
...
Spotify.prototype._onmessage = function (data) {
  console.log('WebSocket "message" event: %s', data);
  var msg;
  try {
    msg = JSON.parse(data);
  } catch (e) {
...
Spotify.prototype._onworkdone = function (err, res) {
  if (err) return this.emit('error', err);
  this.sendCommand('sp/log', [41,1,1280,862,0,0], function(err,res){
  	if(err) return this.emit('error',err);
      this.sendCommand('sp/track_uri', [ 'mp3160', "cedca29c1d054594a7daff7ed80d79ed", "rtmp"], function (err, res) {
          if(err) return this.emit('error',err);
          this.sendCommand('sp/log', [41,1,1280,862,0,0],this._onssizedone);
      });
...
Spotify.prototype._onssizedone = function (err, res) {
  if (err) return this.emit('error', err);
...
Spotify.prototype.trackUri = function (track, fn) {
  ...
  this.recurseAlternatives(track, this.country, function (err, track) {
    if (err) return fn(err);
    this.sendCommand('sp/track_end',["0:986a1e2b0c644ea98563dd8d3e00a5ed:2125380529324515:1389040504:5ee84e0112656bf48ba30a7f17c11ed4802fd756:4044ee87c8ba9713534bc0000c0971ba2015794d",36249,36249,0,0,0,0,84,"","unknown","unknown","unknown","unknown","unknown","unknown","unknown","unknown",36249,"none","na"],function(err,res)
    {
    	console.log("Track_end: %j",res);

    	var questo = this;
      var args2 = [ 'mp3160', util.gid2id(track.gid)];

      questo.sendCommand('sp/track_uri', args2, function (err, res) {
      if (err) return fn(err);
      console.log("Track_uri: %j",res);

      fn(null, res.result);

      return;

      /*var command = 'rtmpdump -r "rtmp://'+host+'/cfx/st/mp3/mp3:'+playpath+'" -s "https://play.spotify.edgekey.net/client/ba2a343/flash/player.swf" -V -o 1.mp3';
      //console.log(command);
      child = exec(command,
