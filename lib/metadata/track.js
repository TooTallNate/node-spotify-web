...
var exportNamespaces = [ PlaySession ];

/** 
 * Re-export namespaces
 */
exportNamespaces.forEach(function (namespace) {
  Track[namespace.name] = namespace;
});
...
/**
 * The list of schemas that the constructor will support 
 */

Track.acceptedSchemas = [ Metadata.schemas.track ];
...
//Track.get = Metadata.get.bind(null, Track);
...
function Track (spotify, uri, parent) {
  ...
  this.acceptedSchemas = Track.acceptedSchemas;
  this.playSession = null;
  this.type = 'track';

  // bind exported namespaces into object
  exportNamespaces.forEach(util.bindNamespace.bind(null, this));
...
Track.prototype.audioUrl = function(format, transport, fn) {
  ...
  this.recurseAlternatives(spotify.user_info.country, function (err, track) {
...
Track.prototype.similar = function(fn) {
  ...
  var request = new spotify.HermesRequest('hm://similarity/suggest/' + id);
  ...
  request.send({
    ...
    var recommendations = res.result.stories.map(function(story) {
      ...
      (function objectify(recommendedItem) {
        var type = util.uriType(recommendedItem.uri);
      ...
      if (data.artist) { 
        ...
        if (story.metadata && story.metadata.summary) track.artist.biography = {text: story.metadata.summary};
        if (story.heroImage) track.artist.portrait = story.heroImage.map(function(image) {
          return {
        ...
        if (track.album) track.album.artist = track.artist;
      }
      return track;
    });
