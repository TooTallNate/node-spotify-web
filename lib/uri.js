...
function SpotifyUri(type, id) {
  ...
  if (!id) {
    ...
    if (/^[0-9a-f]*$/.test(id)) id = base62.fromHex(v, 22);
...
Object.defineProperty(SpotifyUri.prototype, 'user', {
  get: function () {
    ...
    if (len >= 5 && 'user' == parts[1]) return parts[2];
    return null;
  },
  set: function (user) {
    var parts = this._uri_parts;
    ...
    if (len >= 5 && 'user' == parts[1]) parts[2] = user;
  },
  enumerable: true,
  configurable: true
});
...
SpotifyUri.uriType = function (uri) {
  return (new SpotifyUri(uri)).type;
};
